...
async def _do_refresh():
    """Interne Refresh-Logik — Pipeline: Fetch → Build → Post-Analysis."""
    ...
    try:
        # Phase 1: Alle Datenquellen laden + scoren
        fetch_result = await _phase_fetch_sources(_refresh_errors)
        if fetch_result is None:
            return

        stocks, scores_dict, tech_picks, fear_greed_data, eur_usd_rate, is_demo = fetch_result

        # Phase 2: Summary bauen + persistieren
        summary = await _phase_build_summary(
            stocks, scores_dict, tech_picks, fear_greed_data, eur_usd_rate, is_demo,
        )

        # Phase 3: Reports, AI-Kommentare, Telegram, Context Cache
        await _phase_post_analysis(summary, stocks)
...
async def _phase_fetch_sources(_refresh_errors: dict) -> tuple | None:
    """Phase 1: Datenquellen laden (Parqet, FMP, yFinance, F&G) + Scoring.

    Returns (stocks, scores_dict, tech_picks, fear_greed, eur_usd, is_demo) oder None.
    """
    ...
    if is_demo:
        ...
        for pos in positions:
            ...
            score = calculate_score(
                ...
                fundamentals=fund, analyst=analyst,
                current_price=pos.current_price, fmp_rating=fmp_rat,
                yfinance_data=yf, fear_greed=fear_greed_data,
            )
            stocks.append(StockFullData(
                position=pos, fundamentals=fund, analyst=analyst,
                fmp_rating=fmp_rat, yfinance=yf, score=score,
            ))
            ...
            if settings.gemini_configured and tech_picks:
                try:
                    ...
                    tech_picks = await asyncio.wait_for(
                        enrich_with_ai_analysis(tech_picks), timeout=30.0
                    )
                except (asyncio.TimeoutError, Exception) as e:
                    logger.warning(f"Tech-Radar AI: {e}")
        except Exception as e:
            logger.error(f"Tech Picks Fehler: {e}")
            tech_picks = get_demo_tech_picks()

    # Daily Changes + EUR-Konvertierung
    ...
    prices_are_fresh = last_refresh and (datetime.now(tz=TZ_BERLIN) - last_refresh).total_seconds() < 120
    ...
    return stocks, scores_dict, tech_picks, fear_greed_data, eur_usd_rate, is_demo


async def _phase_build_summary(
    stocks, scores_dict, tech_picks, fear_greed_data, eur_usd_rate, is_demo,
) -> "PortfolioSummary":
    """Phase 2: PortfolioSummary bauen, Rebalancing, Snapshot speichern."""
    ...
    summary = PortfolioSummary(
        total_value=t["total_value"], total_cost=t["total_cost"],
        total_pnl=t["total_pnl"], total_pnl_percent=t["total_pnl_pct"],
        num_positions=len(stocks), stocks=stocks,
        scores=[s.score for s in stocks if s.score],
        rebalancing=rebalancing, tech_picks=tech_picks,
        fear_greed=fear_greed_data, is_demo=is_demo,
    ...
    try:
        save_snapshot(
            total_value=t["total_value"], total_cost=t["total_cost"],
            total_pnl=t["total_pnl"], num_positions=len(stocks),
            eur_usd_rate=eur_usd_rate,
        )
    except Exception as e:
...
async def _phase_post_analysis(summary: "PortfolioSummary", stocks: list):
    """Phase 3: Analyse-Report, AI-Kommentare, Telegram, Context Cache."""
    _set_progress("Erstelle Analyse-Report...", 90)

    try:
        report = build_analysis_report(
            stocks_with_scores=stocks, analysis_level="full",
        ...
        if settings.gemini_configured and settings.ENVIRONMENT == "production":
            ...
            if 15 <= now_sc.hour <= 16:
                ...
                logger.info("🤖 Score-Kommentare übersprungen (außerhalb Analysezeit)")

        # Telegram-Report — Zeitfenster-Gate
        now_cet = datetime.now(tz=TZ_BERLIN)
        in_report_window = (
            now_cet.weekday() < 5
            and 15 <= now_cet.hour < 18
            and (now_cet.hour > 15 or now_cet.minute >= 30)
        )
        if settings.telegram_configured and settings.ENVIRONMENT == "production" and in_report_window:
            ...
            logger.info(f"🤖 AI Agent Report übersprungen (außerhalb Zeitfenster, aktuell {now_cet.strftime('%H:%M')})")
        elif settings.telegram_configured:
            logger.info("🤖 AI Agent übersprungen (nur in Production)")
    except Exception as e:
    ...
    if settings.gemini_configured:
        try:
            ...
            await asyncio.wait_for(cache_portfolio_context(summary), timeout=30.0)
        except asyncio.TimeoutError:
